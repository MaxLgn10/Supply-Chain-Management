# tune_ma_window.py
# Finds the globally best MA(n) window across ALL product categories and channels.
...
# ── Grid search ───────────────────────────────────────────────────────────────
results = []
print(f"{'Window':<8} | {'Avg MAPE':>10} | {'Median MAPE':>12} | {'# series':>9}")
print("-" * 48)
...
for n in range(1, MAX_WINDOW + 1):
    ...
    for s in series_list:
        ...
        if len(actual) == 0 or actual[0] == 0 or len(train) < n:
            continue
        pred = float(np.mean(train[-n:]))
        mapes.append(abs(pred - actual[0]) / actual[0] * 100)

    avg    = np.mean(mapes)
    median = np.median(mapes)
    results.append({'n': n, 'avg_mape': avg, 'median_mape': median})
    print(f"MA({n})    | {avg:9.2f}% | {median:11.2f}% | {len(mapes):>9}")

best = min(results, key=lambda x: x['avg_mape'])
print("\n" + "=" * 48)
print(f"Best window: MA({best['n']})")
print(f"  Avg MAPE    = {best['avg_mape']:.2f}%")
print(f"  Median MAPE = {best['median_mape']:.2f}%")
print("=" * 48)
print(f"\n→ Use MA({best['n']}) in all forecasting scripts")
